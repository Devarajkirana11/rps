...
class DashboardManager {
    static get routes() {
        ...
        DashboardRouter.route('/manager/overview')
            ...
            let numberOfDays = end.diff(begin, 'days') + 1;
            ...
            let realroomsdoc = yield roomsCollection
                .find({
                _hotelId: hotelUuid,
                $or: [
                    { _virtualRoom: undefined },
                    { _virtualRoom: false }
                ]
            });
            if (realroomsdoc.length > 0) {
                numberOfRoomNights = Number(realroomsdoc.length) * Number(numberOfDays);
            }
            let real_room_nos = new Array();
            for (let realroom of realroomsdoc) {
                real_room_nos.push(realroom._number);
            }
            let maintenanceDocuments = yield roomActivityCollection
                .find({
                hotelUuid: hotelUuid,
                blockingIssues: {
                    $elemMatch: {
                        "duration.startDate": { $not: { $gt: end.toDate() } },
                        "duration.endDate": { $not: { $lt: begin.toDate() } }
                    }
                },
                roomNumber: { $in: real_room_nos }
            });
            for (let maintain of maintenanceDocuments) {
                for (let blockeddates of maintain.blockingIssues) {
                    let blockingStartDate = time_1.default.serverMomentInPattern(blockeddates.duration.startDate, "YYYY-MM-DD HH:mm:ss");
                    let blockingEndDate = time_1.default.serverMomentInPattern(blockeddates.duration.endDate, "YYYY-MM-DD HH:mm:ss");
            ...
            let roomsSold = 0;
            for (let occupied of roomReservationDocuments) {
            ...
            yield roomReservationCollection.find(reservationCondition, { "sort": { "_id": -1 } }).then((revenueDocuments) => __awaiter(this, void 0, void 0, function* () {
            ...
            let roomTypeMaintenanceDocuments = yield roomActivityCollection.find({
                hotelUuid: hotelUuid,
                blockingIssues: {
                    $elemMatch: {
                        "duration.startDate": { $not: { $gt: end.toDate() } },
            ...
            for (let maintain of roomTypeMaintenanceDocuments) {
                for (let blockeddates of maintain.blockingIssues) {
                    ...
                    let blockingEndDate = time_1.default.serverMomentInPattern(blockeddates.duration.endDate, "YYYY-MM-DD HH:mm:ss");
                    if (begin >= blockingStartDate && end >= blockingEndDate) {
                        blockeddates.duration.startDate = begin.toDate();
                    }
                    else if (begin <= blockingStartDate && end <= blockingEndDate) {
                        blockeddates.duration.endDate = end.toDate();
                    }
                    else if (begin >= blockingStartDate && end <= blockingEndDate) {
                        blockeddates.duration.startDate = begin.toDate();
                        blockeddates.duration.endDate = end.toDate();
            ...
            for (let typeObj of roomTypeStats) {
                ...
                let roomTypeFilter = roomTypeMaintenanceDocuments.filter(e => Number(e.roomType) === typeObj.roomType);
                let uniqueRoomTypes = yield roomTypeFilter.map(room => room.roomNumber).filter((value, index, self) => self.indexOf(value) === index);
                for (let unique of uniqueRoomTypes) {
                    let filteredRooms = roomTypeFilter.filter(e => Number(e.roomNumber) === Number(unique));
                    for (let filteredRoom of filteredRooms) {
                        for (let blockeddates of filteredRoom.blockingIssues) {
                            let blockedStartDate = time_1.default.serverMomentInPattern(blockeddates.duration.startDate, "YYYY-MM-DD HH:mm:ss");
                            let blockedEndDate = time_1.default.serverMomentInPattern(blockeddates.duration.endDate, "YYYY-MM-DD HH:mm:ss");
            ...
            for (let booking in InventoryLaw.BookingSource) {
                ...
                for (let rooms of filteredRooms) {
                    for (let cost of rooms.costBreakdown) {
                        if (cost.date >= begin.toDate() && cost.date <= end.toDate()) {
        ...
        DashboardRouter.route('/manager/revenue')
            ...
            let maintenanceDocuments = yield roomActivityCollection.find({
                hotelUuid: hotelUuid,
                blockingIssues: {
                    $elemMatch: {
                        "duration.startDate": { $not: { $gt: end.toDate() } },
            ...
            let summaryMaintenanceDocuments = yield roomActivityCollection.find({
                hotelUuid: hotelUuid,
                blockingIssues: {
                    $elemMatch: {
                        "duration.startDate": { $not: { $gt: end.toDate() } },
                ...
                case 'revPar': {
                    ...
                    let summaryMaintenanceDocuments = yield roomActivityCollection.find({
                        ...
                        blockingIssues: {
                            $elemMatch: {
                                "duration.startDate": { $not: { $gt: end.toDate() } },
                                "duration.endDate": { $not: { $lt: monthStart.toDate() } }
                            }
                        }
                    });
                    let summaryMaintenanceList = new Array();
